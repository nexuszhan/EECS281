...
void testPrimitiveOperations() {
    ...
    std::vector<int> const vect{1,3,5,2};
    PQ<int> eecsPQ2(vect.cbegin(), vect.cend());
    assert(eecsPQ2.top() == 5);
    assert(eecsPQ2.size() == 4);
    eecsPQ2.pop();
    eecsPQ2.pop();
...
void testHiddenData() {
    ...
    struct HiddenDataComp {
        bool operator()(const HiddenData & h1, const HiddenData & h2) const {
    ...
    eecsPQ.push(hd2);
    assert(eecsPQ.size() == 2);
    assert(eecsPQ.top().data == hd2.data && eecsPQ.top().name == hd2.name);
...
void testPairing() {
...

    {
        ...
        PairingPQ<int>::Node* node1 = pairing1.addNode(2);
        assert(pq1.size() == 4);
        pairing1.updateElt(node1, 5);
        assert(pq1.top() == 5);
        assert(pq1.size() == 4);
        pq1.pop();
        pairing1.updateElt(pairing1.addNode(2), 5);
        assert(pq1.top() == 5);
        ...
        assert(pq4.empty());
        PairingPQ<int>::Node* node2 = pairing4.addNode(0);
        pq4.push(2);
        pq4.push(1);
        pq4.push(4);
        pq4.push(3);
